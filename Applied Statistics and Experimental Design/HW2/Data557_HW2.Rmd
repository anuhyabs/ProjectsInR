---
title: "Data557 Homework Assignment 2"
author: "Anuhya B S"
...
# p = 0.0107
```
However, since the question states that there is no value in knowing whether there is a decrease in output when temperature is increased it makes sense to only consider one half of the p-value which is 0.0054.
```{r}
p = round((pnorm(z)),4)
p
```
...
Applying Bonferroni correction to the results of the previous question,
...
```{r}
method <- as.factor(data_defect$Method)
summary(aov(data_defect$Weight~method))
```
